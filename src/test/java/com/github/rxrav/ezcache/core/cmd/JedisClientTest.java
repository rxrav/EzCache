...
public class JedisClientTest {
    ...
    void shouldPing() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            logger.info("Ping test, received: {}", actual);
    ...
    void shouldEcho() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            logger.info("Echo test, received: {}", actual);
    ...
    void shouldSet() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            logger.info("Set test, received: ", actual);
    ...
    void shouldSetWithExThenWaitThenGetNull() throws InterruptedException {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            logger.info("Set with ex test, received: {}", actual);
            assertEquals("OK", actual);

            actual = jedis.get("name-ex");
            logger.info("Get test, received: {}", actual);
            ...
            actual = jedis.get("name-ex");
            logger.info("Get test (again) to check expired, received: {}", actual);
            assertNull(actual);
    ...
    @Test
    void shouldNotSetWithExNegativeTime() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            SetParams params = new SetParams();
            params.ex(-4);
            JedisDataException ex = assertThrows(JedisDataException.class, () -> jedis.set("name-ex", "jim", params));
    ...
    @Test
    void shouldNotSetWithPxNegativeTime() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            SetParams params = new SetParams();
            params.px(-4);
            JedisDataException ex = assertThrows(JedisDataException.class, () -> jedis.set("name-px", "jim", params));
    ...
    void shouldSetWithPxThenWaitThenGetNull() throws InterruptedException {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            logger.info("Set with px test, received: {}", actual);
            assertEquals("OK", actual);
    ...
    void shouldThrowErrorForSetWithBadOpt() {
        String badOpt = "qx";
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            JedisDataException ex = assertThrows(JedisDataException.class, () -> jedis.sendCommand(
    ...
    void shouldThrowErrForIncr() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            JedisDataException ex = assertThrows(JedisDataException.class, () -> jedis.incr("incr-ex"));
    ...
    void shouldThrowErrForDecr() {
        try(Jedis jedis = new Jedis("127.0.0.1", 6379)) {
            ...
            JedisDataException ex = assertThrows(JedisDataException.class, () -> jedis.decr("decr-ex"));
            assertEquals("WRONGTYPE Not a valid number type", ex.getMessage());
        } catch (Exception e) {
            logger.error(e.getMessage());
            throw e;
